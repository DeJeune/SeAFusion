...
# def convert_to_onnx(net, save_path):
#     """将模型转换为ONNX格式"""
#     net.eval()
...
def convert_to_onnx(net, save_path, model_name=""):
    """使用 TorchDynamo 导出器"""
    net.eval()
    
    # 创建示例输入
    if model_name == "fusion":
        dummy_vis = torch.randn(1, 3, 480, 640)
        dummy_ir = torch.randn(1, 1, 480, 640)
    else:
        dummy_input2 = torch.randn(1, 3, 480, 640)
    ...
    print(f"Starting {model_name} model export...")
    try:
        if model_name == "fusion":
...
if __name__ == '__main__':
    ...
    model_dir = './model/Fusion'
    os.makedirs(model_dir, exist_ok=True)
    
    # 转换分割模型
    print("Converting segmentation model...")
    seg_model = BiSeNet(n_classes=9)
    weights = torch.load(os.path.join(model_dir, 'model_final.pth'), 
                                       map_location='cpu')
    seg_model.load_state_dict(weights)
    seg_model.eval()
    convert_to_onnx(seg_model, 
                    os.path.join(model_dir, 'model_final.onnx'),
                    "segmentation")
    
    # 转换融合模型
    print("\nConverting fusion model...")
    fusion_model = eval('FusionNet')(output=1)
    
    # 尝试兼容模式加载权重
    state_dict = torch.load(os.path.join(model_dir, 'fusion_model.pth'), 
                          map_location='cpu')
    for k, v in state_dict.items():
        # 打印旧键
        print(k)
    # 创建新的 state_dict，只加载匹配的键
    new_state_dict = {}
    for k, v in fusion_model.state_dict().items():
        if k in state_dict:
            new_state_dict[k] = state_dict[k]
        else:
            print(f"Warning: {k} not found in checkpoint, using random initialization")
            new_state_dict[k] = v
    
    # 加载处理后的权重
    fusion_model.load_state_dict(new_state_dict, strict=False)
    fusion_model.eval()
    
    convert_to_onnx(fusion_model, 
                    os.path.join(model_dir, 'fusion_model.onnx'),
                    "fusion")
    
    print("\nAll models conversion completed!")
    # 创建新的 state_dict，只加载匹配的键
    new_state_dict = {}
    for k, v in fusion_model.state_dict().items():
        if k in state_dict:
...
#     seg_model.load_state_dict(torch.load(os.path.join(model_dir, 'model_final.pth'), 
#                                        map_location='cpu'))
#     convert_to_onnx(seg_model, os.path.join(model_dir, 'model_final.onnx'))
    
#     # 转换融合模型
#     fusion_model = eval('FusionNet')(output=1)
#     fusion_model.load_state_dict(torch.load(os.path.join(model_dir, 'fusion_model.pth'), 
#                                           map_location='cpu'))
#     convert_to_onnx(fusion_model, os.path.join(model_dir, 'fusion_model.onnx'))
